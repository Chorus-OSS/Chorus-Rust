...
static INSTANCE: Lazy<RwLock<BlockTypeRegistry>> =
    Lazy::new(|| RwLock::new(BlockTypeRegistry::create()));
...
impl BlockTypeRegistry {
    fn create() -> Self {
        ...
        registry
            .register(block_id::AIR, Lazy::new(|| Air::TYPE.clone()))
            .unwrap();
        registry
            .register(block_id::DIRT, Lazy::new(|| Dirt::TYPE.clone()))
            .unwrap();
        registry
            .register(
                block_id::GRASS_BLOCK,
                Lazy::new(|| GrassBlock::TYPE.clone()),
            )
            .unwrap();
        registry
            .register(block_id::BEDROCK, Lazy::new(|| Bedrock::TYPE.clone()))
            .unwrap();
